...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   linux_hooks_render.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: lzins <marvin@42.fr>                       +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/08 13:50:52 by lzins             #+#    #+#             */
/*   Updated: 2021/03/08 13:53:13 by lzins            ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	motion_hook(int x, int y, t_pack *p)
{
	...
	if (firstcall)
	{
		mlx_mouse_move(p->mlx, p->win, p->width / 2, p->height / 2);
		firstcall = 0;
		return (0);
	}
	delta_x = x - p->width / 2;
	delta_y = y - p->height / 2;
	rotate3d_packdir(p, -MOUSE_SENSIB * delta_x, MOUSE_SENSIB * delta_y);
	mlx_mouse_move(p->mlx, p->win, p->width / 2, p->height / 2);
	return (0);
}
